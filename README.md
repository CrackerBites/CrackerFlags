...
```
> [!NOTE]
> i recommend removing the following `modules=jdk.incubator.vector` if you dont use a fork of pufferfish of pufferfish itself otherwise its useless having it there and also `-DIReallyKnowWhatIAmDoingISwear` this is an spigot only flag remove it if you want to get notified if you are using an old build this skips the checks for a faster startup`
